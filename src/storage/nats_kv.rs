...
use async_nats::{
    ...
    jetstream::kv::Config as JetstreamKvConfig,
    jetstream::kv::Store as JetstreamKvStore,
};
...
pub struct NatsKvStorageConfig {
    ...
    /// Prefix to prepend to NATS bucket name
    pub nats_prefix: Option<String>,
}

impl NatsKvStorageEngine {
    #[allow(dead_code)]
    pub async fn new(config: NatsKvStorageConfig) -> Result<NatsKvStorageEngine, StorageEngineError> {
        // Connect to NATS
        let client = async_nats::connect(&config.nats_url).await
            .map_err(|e| StorageEngineError::Engine(format!("failed to connect: {e}")))?;

        Ok(NatsKvStorageEngine {
            config,
            client,
        })
    }
    ...
    fn build_lattice_bucket_name(&self, lattice_id: String) -> String {
        ...
        if let Some(prefix) = &self.config.nats_prefix {
    ...
    async fn create_kv_store(&self, bucket: String) -> Result<JetstreamKvStore, StorageError> {
        // Build jetstream client
        let jetstream = async_nats::jetstream::new(self.client.clone());

        // Create Jetstream KV store
        let kv = jetstream.create_key_value(JetstreamKvConfig {
            bucket,
            history: 10,
            ..Default::default()
        }).await
...
impl Store for NatsKvStorageEngine {
    ...
    async fn store(&mut self, state: LatticeState, store_opts: StoreOptions) -> Result<WithStateMetadata<String>, StorageError> {
    ...
    async fn delete(&self, id: String) -> Result<(), StorageError> {
        ...
        let kv = self.create_kv_store(bucket.clone()).await?;

        let _ = kv.delete(bucket)
            .await
            .map_err(|err| StorageError::Engine(format!("failed to delete lattice state: {err}")))?;
...
impl StoreRead<LatticeState,String> for NatsKvStorageEngine {
    async fn read(&self, id: String) -> Result<WithStateMetadata<LatticeState>, StorageError> {
        ...
        match kv.entry(LATTICE_BUCKET_STATE_KEY).await? {
            ...
            None =>  {
                Err(StorageError::Engine(format!("No lattice state in bucket {bucket}")))
            }
        }
    }
}
...
impl StoreWrite<LatticeState,String> for NatsKvStorageEngine {
    async fn write(
        ...
        match opts {
            ...
            StoreOptions { require_cas: true, revision: Some(rev), .. } => {
                revision = kv.update(LATTICE_BUCKET_STATE_KEY, to_vec(&obj)?.into(), rev).await?;
            },

            // Best-effort semantics
            _ => {
                revision = kv.put(LATTICE_BUCKET_STATE_KEY, to_vec(&obj)?.into()).await?;
            }
        }
