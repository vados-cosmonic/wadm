...
///
/// Note that these msut be this way because we can't write non-overlapping blanked impls yet:
...
pub trait StoreWrite<State,Id> {
    ...
    async fn write(&mut self, obj: &State, id: Option<Id>, opts: StoreOptions) -> Result<WithStateMetadata<Id>, StorageError>;
}
    ...
    Self: StoreRead<Self::State, Self::StateId> + StoreWrite<Self::State, Self::StateId>,
{
    ...
    async fn store(&mut self, state: Self::State, opts: StoreOptions) -> Result<WithStateMetadata<Self::StateId>, StorageError>;
...
pub trait LatticeStorage: Store<State=LatticeState, StateId=String, StateUpdate=LatticeStateBuilder> {}
