...
// Link puts take a slightly longer time to work through
const LINK_PUT_TIMEOUT_DURATION: Duration = Duration::from_secs(20);
...
async fn test_event_stream() -> Result<()> {
    ...
    let mut evt = wait_for_event(&mut stream, LINK_PUT_TIMEOUT_DURATION).await;
...
async fn wait_for_event(
    ...
    let mut evt = timeout(duration, stream.try_next())
        .await
        .expect(format!("Should have received event before timeout {}s", duration.as_secs()).as_str())
