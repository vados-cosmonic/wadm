...
const DEFAULT_NATS_DOCKER_IMAGE: &str =
    "nats:2.9.15-scratch@sha256:1a82b1b261d169994fc6162f2f25894a49e20971560f8b2dff4f90eefb32b421";
    ...
    loop {
        ...
        if tries >= 5 {
            return Err(Error::msg(format!(
                "Failed to find port after trying {NUM_PORT_TRIES} times"
            )));
        }
    }
...
pub(crate) async fn make_nats_instance() -> Result<Box<dyn NatsInstance>> {
    ...
    let docker_disabled = env::var(ENV_VAR_DOCKER_DISABLED)
        .map(|v| v == "true")
        .unwrap_or(DEFAULT_DOCKER_DISABLED);
...
impl ProcessNats {
    async fn new(port: u16) -> Result<ProcessNats> {
        ...
        // Create temporary directory to store NATS state in
        ...
        let child = Command::new(&nats_bin_path)
            ...
            .arg(port.to_string())
            .arg("-sd")
            .arg(storage_dir.into_path())
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::null())
            .spawn()
            .map_err(|e| {
                Error::msg(format!(
                    "nats command ({nats_bin_path}) failed to spawn: {e}"
                ))
            })?;
...
impl DockerizedNats {
    async fn new(port: u16) -> Result<DockerizedNats> {
        ...
        let nats_image =
            env::var(ENV_VAR_NATS_DOCKER_IMAGE).unwrap_or(DEFAULT_NATS_DOCKER_IMAGE.into());

        let container_name = format!(
            "nats-test-{}",
            generate_random_string(6, CONTAINER_SUFFIX_ALPHABET)
        );
        ...
        let config = ContainerConfig {
            ...
            cmd: Some(vec!["-js".into(), "-p".into(), port.to_string()]),
            exposed_ports: Some(HashMap::from([(format!("{}", &port), HashMap::new())])),
            ..Default::default()
        };
